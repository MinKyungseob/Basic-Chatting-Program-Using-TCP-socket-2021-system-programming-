...
void* send_msg(void* arg)
{
    ...
    while(1)
    {
        ...
        // send message
        if(name_change_sign == 1)
        {
            sprintf(name_msg, "%s changed to %s %s", name_temp,name,msg);
            name_change_sign--;
            sprintf(name_temp, "[%s]", name);
        }
        else
...
{
    system("clear");
    printf(" **** client information of %s ****\n", name);
